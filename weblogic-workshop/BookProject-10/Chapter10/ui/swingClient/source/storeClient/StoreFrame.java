...
/* 
 * This class is designed merely to display table with data from the web
...
public class StoreFrame extends javax.swing.JFrame {
	...
	/*
	 * Declare variables to hold the proxy classes and the URL that 
	 * points to the web service. Of course, in a real application,
	 * the URL’s host name and port number would not likely belong to 
	 * the local server.
	 */
	...
	/*
	 * Variables to hold the Item array returned by the viewCatalog
	 * method, and to represent the JTable and JScrollPane that we’ll
	...
	public void initComponents() throws Exception {
	    /* 
	     * Call the createProxy and generateConvID methods to initialize 
	     * the proxy classes and conversation ID value.
	     */
		createProxy();
		generateConvID();

	    /* 
	     * Initialize variable for the start header and continue values. 
	     * These use the conversation ID assigned to a member variable 
	     * from generatedConvID.
	     */
		sh = new StartHeader(convID, "http://localhost:7001/OnlineStorePoll.jws");
	    ch = new ContinueHeader(convID);

	    /* 
	     * Initialize the catalog table using the TableModel object
	     * returned by the generateCatalogTable method. Insert the 
	     * new table into a JScrollPane.
	     */
		tblCatalog = new javax.swing.JTable(generateCatalogTable());
		scrCatalog = new javax.swing.JScrollPane(tblCatalog);

	    /*
	     * The remaining code in this method sets up other user interface elements
	     * used by the application, the pull it all together in the 
	     * window.
	     */
	...
	/* 
	 * Create the proxy object that will be used to communicate with the 
	 * web service.
	 */  
	...
	public TableModel generateCatalogTable () {
		try {
		        /* 
		         * Call the startShopping method with a user name and 
		         * and customer number passed from a login dialog box 
		         * (that code is not shown here). This starts the 
		         * conversation by passing a StartHeader (initialized
		         * in code later in this section).
		         */ 
	    		greeting = storeProxy.startShopping(userName, 
	    			customerNumber, sh);

	        /*
	         * Call the viewCatalog method to retrieve the list of
	         * items in the catalog. Notice that a ContinueHeader is 
	         * passed with the method call.
	         */
			catalogItems = storeProxy.viewCatalog(ch);			
	    	} catch (java.rmi.RemoteException re) {
				/* 
		          * Code to retrieve the contents of the error detail
		          * and display an error message the user.
		          */
			}
	  
		    /* 
		     * Declare an array of Object arrays to use for filling the 
		     * table with the list of items. We’ll assign a value from a 
		     * method that converts the Item array returned by the web 
		     * service to the type we need here. Code for the method 
		     * follows.
		     */
	    	final Object[][] itemsList = convertItems(catalogItems);
	    	
		    /* 
		     * Create a new TableModel object that will be used to create a 
		     * JTable.
		     */
	    	TableModel catalogTable = new AbstractTableModel() {
		        /* 
		         * Set the table’s values to the contents of the array
		         * of Object arrays.
		         */
		        ...
		        /* 
		         * Implement methods of the AbstractTableModel interface
		         * using the data and headers variables.
		         */
	...
	public Object[][] convertItems (Item[] items) {
	    /* 
	     * Declare an array of Object arrays, giving it values
	     * corresponding to the number of items in the list (for 
	     * table rows) and the number of fields in the Item class
	     * for table columns.
	     */
		Object[][] itemsArray = new Object[items.length][3];
		    /*
		     * Loop through the array of items, turning each one into
		     * an array whose members are its code, name, and price fields.
		     */
